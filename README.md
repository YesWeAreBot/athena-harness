...
- **Life / Mode / Body Lifecycle**: Life can attach Bodies, switch Modes, route Percepts, and dispose safely under plugin hot unload.
- **Memory / Scheduler / AgentLoop Infrastructure**: Replaceable `LifeMemory`, `Scheduler`, and `AgentLoopRegistry` contracts are available to Mode developers.
- **BodyAdapter Contract**: Existing Koishi adapters such as OneBot/Satori can be bridged into Life Bodies through `PerceptEvent` and `Actuator`.
...
This is the current Athena Runtime prototype: create a Life named Athena, attach a Body, activate a Mode, and route a Percept. Memory, Scheduler, AgentLoop provider, and BodyAdapter infrastructure are now available as early contracts; automatic Life-to-AgentLoop wiring and production Memory remain future work.
