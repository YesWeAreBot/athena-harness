...
export const escapeMarkdown = (val: string) => val.replace(/([\\`*_[*_~`\]\-(#!>])/g, "\\$&");
    ...
    try {
      if (useFormData) {
        ...
        const payload: QQ.Message.ChannelRequest = {
          ...
          msg_id,
          image: this.fileUrl,
          ...(this.reference ? { message_reference: { message_id: this.reference } } : {}),
          ...(this.passiveEventId ? { event_id: this.passiveEventId } : {}),
        };
      ...
      if (new TextDecoder().decode(data.slice(0, 7)).includes("#!SILK")) {
        ...
        if (ntsilk) {
          ...
          if (silk.isWav(data) && allowSampleRate.includes(silk.getWavFileInfo(data).fmt.sampleRate)) {
            ...
            const pcmBuf = await this.bot.ctx
              .get("ffmpeg")
              .builder()
