...
export class QQGuildBot extends Bot {
  ...
  async createChannel(guildId: string, data: Partial<Universal.Channel>) {
    const channel = await this.internal.createGuildChannel(guildId, {
      name: data.name,
      type:
        data.type === Universal.Channel.Type.TEXT
          ? QQ.ChannelType.TEXT
          : data.type === Universal.Channel.Type.CATEGORY
            ? QQ.ChannelType.GROUP
            : data.type === Universal.Channel.Type.VOICE
              ? QQ.ChannelType.VOICE
              : QQ.ChannelType.TEXT,
