...
export async function downloadFile(http: HTTP, url: string) {
  ...
  const filename = match ? decodeURIComponent(match[1]) : new URL(url, "http://localhost").pathname.split("/").pop() || "file";
...
export const decodeChannel = (channel: QQ.Channel): Universal.Channel => ({
  ...
  name: channel.name,
  type:
    channel.type === QQ.ChannelType.TEXT
      ? Universal.Channel.Type.TEXT
      : channel.type === QQ.ChannelType.VOICE
        ? Universal.Channel.Type.VOICE
        : channel.type === QQ.ChannelType.GROUP
          ? Universal.Channel.Type.CATEGORY
          : Universal.Channel.Type.TEXT,
...
export function decodeGroupMessage(bot: QQBot, data: QQ.UserMessage, message: Universal.Message = {}, payload: Universal.MessageLike = message) {
  message.id = data.id;
  message.elements = [];
  ...
  if (date.includes("m=")) {
    date = data.timestamp
      .slice(0, data.timestamp.indexOf("m="))
      .trim()
...
export async function decodeMessage(
  ...
  message.id = data.id;
  message.content = (data.content ?? "").replace(/<@!(\d+)>/g, (_, $1) => at($1).toString());
...
export async function adaptSession(bot: QQBot, input: QQ.DispatchPayload) {
  ...
  if (
    !["GROUP_AT_MESSAGE_CREATE", "C2C_MESSAGE_CREATE", "FRIEND_ADD", "FRIEND_DEL", "GROUP_ADD_ROBOT", "GROUP_DEL_ROBOT", "INTERACTION_CREATE"].includes(input.t)
  ) {
