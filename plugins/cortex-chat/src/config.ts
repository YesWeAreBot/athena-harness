...
export const CortexChatConfigSchema: Schema<CortexChatConfig> = Schema.object({
  ...
  initialFocus: Schema.string().default("").description("启动时默认 focus 场景（platform:channelId），留空则首条 trigger 自动设定"),
  ...
  historyLimit: Schema.number().default(30).description("focus 展开时从 message-store 加载的最大消息数"),
});
